    ...
    <div class="prof-grid">
      <div>
        ...
        <div class="prof-section">
          ...
          ${revs.length ? revs.map(reviewHTML).join("") : `<p class="page-lead" style="margin:0">No written reviews yet — be the first to work with ${esc(pr.name.split(" ")[0])}.</p>`}
        </div>
      </div>
      ...
      <aside>
        <div class="hire-card">
          ...
          <button class="btn btn-gold btn-block btn-lg" data-hire="${pr.id}">Hire ${esc(pr.name.split(" ")[0])}</button>
...
function applicationSentHTML(name) {
  return `<div class="modal-pad" style="text-align:center">
    ...
    <p class="modal-sub" style="margin-top:10px">Thanks, ${esc(name.split(" ")[0])}. Your profile is now in the admin review queue. Once it's approved, you'll appear in Browse and can start receiving jobs.</p>
...
function customerJobHTML(j) {
  ...
  let actions = "";
  if (j.status === "requested") actions = `<span class="job-sub">Waiting for ${esc(j.providerName.split(" ")[0])} to accept</span>`;
  else if (j.status === "accepted") actions = `<span class="job-sub">${esc(j.providerName.split(" ")[0])} is on it</span>`;
  ...
  openModal(`
    <div class="modal-pad">
      <div class="modal-head">
        <div><h2>Hire ${esc(pr.name.split(" ")[0])}</h2></div>
      ...
      <div class="field"><label>What do you need done?</label><input id="hTitle" type="text" placeholder="e.g. Fix a leaking kitchen tap" /></div>
      <div class="field"><label>Details</label><textarea id="hDetails" rows="3" placeholder="Anything ${esc(pr.name.split(" ")[0])} should know"></textarea></div>
      <div class="field-row">
      ...
      <button class="btn btn-gold btn-block btn-lg" data-send-hire="${pr.id}">Send job request</button>
    </div>`);
  // live total
...
function sendHire(providerId) {
  ...
  save();
  closeModal();
  toast(`Request sent to ${pr.name.split(" ")[0]}. Track it in your dashboard.`, "success");
}
...
function confirmPay(id) {
  ...
  toast(`Payment of ${money(j.amount)} released to ${j.providerName.split(" ")[0]}`, "success");
}
...
function openReviewModal(jobId) {
  ...
  openModal(`
    <div class="modal-pad">
      <div class="modal-head"><div><h2>Rate ${esc(j.providerName.split(" ")[0])}</h2></div><button class="modal-close" data-close>×</button></div>
...
function saveIdentity() {
  ...
  closeModal();
  renderIdentityDependent();
  toast(v ? `Hi ${v.split(" ")[0]} 👋` : "Signed out to Guest");
}
